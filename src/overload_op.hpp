#ifndef NT_NAMED_ARGUMENT_PARAMETERS_OVERLOAD_OP_HPP__
#define NT_NAMED_ARGUMENT_PARAMETERS_OVERLOAD_OP_HPP__
...
struct nt_auto_return_{static constexpr std::size_t Index = N;};

template<typename T> struct _isnt_auto_return_ : std::false_type{};
template<std::size_t N> struct _isnt_auto_return_<nt_auto_return_<N>> : std::true_type{};
template<typename OutArg, typename Tuple>
struct nt_extract_auto_return_type_{using type = OutArg;};
template<std::size_t N, typename Tuple>
struct nt_extract_auto_return_type_<nt_auto_return_<N>, Tuple> {using type = type_details::remove_cvref_t<std::tuple_element_t<N, Tuple>>;};
template<std::size_t N, typename Tuple>
struct nt_extract_auto_return_type_<nt_auto_return_<N>&, Tuple> {using type = type_details::remove_cvref_t<std::tuple_element_t<N, Tuple>>&;};
template<std::size_t N, typename Tuple>
struct nt_extract_auto_return_type_<const nt_auto_return_<N>&, Tuple> {using type = const type_details::remove_cvref_t<std::tuple_element_t<N, Tuple>>&;};
template<std::size_t N, typename Tuple>
struct nt_extract_auto_return_type_<const nt_auto_return_<N>, Tuple> {using type = const type_details::remove_cvref_t<std::tuple_element_t<N, Tuple>>;};
...
//checks if a type is an nt_convertible_
template<typename T>
struct _isnt_convertable_ : std::false_type {};

template<typename T>
struct _isnt_convertable_<nt_convertible_<T> > : std::true_type {};


template<typename Arg, typename TArg>
inline static constexpr bool nt_is_arg_callable_value_ = 
    (std::is_same_v<Arg, TArg> 
    ||  std::is_same_v<Arg, nt_auto_>
    ||  (std::is_reference_v<TArg> && std::is_reference_v<Arg> && 
        ((std::is_same_v<type_details::remove_cvref_t<Arg>, type_details::remove_cvref_t<TArg>>) || std::is_same_v<type_details::remove_cvref_t<Arg>, nt_auto_>)
         && ((!std::is_const_v<Arg> && !std::is_const_v<TArg>) || std::is_const_v<Arg>))
    || (std::is_reference_v<Arg> && std::is_const_v<Arg>)
    || (std::is_reference_v<TArg> && !std::is_reference_v<Arg>)
    || _isnt_convertable_<Arg>::value);


//checks if all the types from an overload are callable
template<typename Arg1, typename... Args>
struct nt_is_callable_{
    
    template<typename Tuple>
    inline static constexpr bool value(){
        //nt_remove_first_tuple_type_impl<Tuple> is just going to get rid of the first tuple type
        if constexpr (std::tuple_size_v<Tuple> == 0){return true;}
        else{
            using TArg1 = std::tuple_element_t<0, Tuple>;
            if constexpr (nt_is_arg_callable_value_<Arg1, TArg1>){
                if constexpr (sizeof...(Args) == 0) return true;
                else return nt_is_callable_<Args...>::template value<typename nt_remove_first_tuple_type_impl<Tuple>::type>();
            }else{
                return false;
            }
    ...
    inline static constexpr bool convertible_value(){
        if constexpr (std::tuple_size_v<Tuple> == 0){return true;}
            ...
            if constexpr (std::is_convertible_v<TArg1, Arg1>){
                if constexpr (sizeof...(Args) == 0){return true;}
                else return nt_is_callable_<Args...>::template convertible_value<typename nt_remove_first_tuple_type_impl<Tuple>::type>();
            }else if constexpr (std::is_same_v<nt_auto_, Arg1>){
                if constexpr (sizeof...(Args) == 0){return true;}
                else return nt_is_callable_<Args...>::template convertible_value<typename nt_remove_first_tuple_type_impl<Tuple>::type>();
            }
            else if constexpr (_isnt_convertable_<Arg1>::value){
                if constexpr (sizeof...(Args) == 0) {return true;}
                else {return Arg1::template is_converible<TArg1>() && nt_is_callable_<Args...>::template convertible_value<typename nt_remove_first_tuple_type_impl<Tuple>::type>();}
            }
            else{
                return false;
            }
...
struct nt_const_volatile_ref_like_{
...
template<typename T1, typename T2>
struct nt_fix_singular_arg_holder_type_{
    using type = std::conditional_t<std::is_same_v<type_details::remove_cvref_t<T2>, nt_auto_>, typename nt_const_volatile_ref_like_<T2, T1>::type, T2>;
};

template<typename T1, typename CT>
struct nt_fix_singular_arg_holder_type_<T1, nt_convertible_<CT>>{
    using type = CT;
};
...
struct nt_fix_arg_holder_;

template<typename Tup1, typename Tup2, std::size_t... Is>
struct nt_fix_arg_holder_<Tup1, Tup2, std::index_sequence<Is...>>{
    ...
    using type = std::tuple<typename nt_fix_singular_arg_holder_type_<std::tuple_element_t<Is, Tup1>, std::tuple_element_t<Is, Tup2>>::type...>;
};



// template<std::size_t i, typename Tup1, typename Tup2>
// inline constexpr auto nt_fix_arg_holder_(){
//     if constexpr(std::tuple_size_v<Tup1> <= i || std::tuple_size_v<Tup2> <= i){
//         return Tup1{};
//     }
//     else{
//         using out_type = typename nt_fix_singular_arg_holder_type_<std::tuple_element_t<i, Tup1>, std::tuple_element_t<i, Tup2>>::type;
//         if constexpr (std::is_same_v<out_type, std::tuple_element_t<i, Tup1>>){
//             return nt_fix_arg_holder_<i+1, Tup1, Tup2>();
//         }
//         else{
//             using replace_type = nt_replace_tuple_type_t<i, out_type, Tup1>;
//             return nt_fix_arg_holder_<i+1, replace_type, Tup2>();
            
//         }
//         // if constexpr (_isnt_convertable_<std::tuple_element_t<i, Tup2>>::value){
//         //     using replace_type = nt_replace_tuple_type_t<i, typename std::tuple_element_t<i, Tup2>::Type, Tup1>;
//         //     return nt_fix_arg_holder_<i+1, replace_type, Tup2>();
//         // }
...
//         //     using replace_type = nt_replace_tuple_type_t<i, std::tuple_element_t<i, Tup2>, Tup1>;
//         //     return nt_fix_arg_holder_<i+1, replace_type, Tup2>(); 
//         // }
//         // else if constexpr(std::is_same_v<std::tuple_element_t<i, Tup2>, nt_auto_&>){
...
//         //     using replace_type = nt_replace_tuple_type_t<i, absolute_type, Tup1>;
//         //     return nt_fix_arg_holder_<i+1, replace_type, Tup2>(); 
//         // }
//         // else if constexpr(std::is_same_v<std::tuple_element_t<i, Tup2>, const nt_auto_&>){
...
//         //     return nt_fix_arg_holder_<i+1, replace_type, Tup2>(); 
//         // }
//         // else if constexpr(std::is_same_v<std::tuple_element_t<i, Tup2>, const volatile nt_auto_&>){
...
//         //     return nt_fix_arg_holder_<i+1, replace_type, Tup2>(); 
//         // }
//         // else if constexpr(std::is_same_v<std::tuple_element_t<i, Tup2>, volatile nt_auto_&>){
//         //     static_assert(std::is_reference_v<std::tuple_element_t<i, Tup1>>
//         //                 && !std::is_const_v<std::tuple_element_t<i, Tup1>>,
//         //                   "Erorr, unable to convert type to non-const reference");
...
template<typename... Args>
struct _hasnt_auto_ : std::false_type {};

template<typename Arg, typename... Args>
struct _hasnt_auto_ <Arg, Args...>{
    static constexpr bool value = std::is_same_v<Arg, nt_auto_> || _hasnt_auto_<Args...>::value;
};


template<typename... Args>
struct _hasnt_convertible_ : std::false_type {};

template<typename Arg, typename... Args>
struct _hasnt_convertible_<Arg, Args...>{
    static constexpr bool value = _isnt_convertable_<Arg>::value || _hasnt_convertible_<Args...>::value;
};


template<typename OutArg, typename... ParamArgs>
struct nt_overload_operator_;

template<typename OutArg, typename Tuple>
struct nt_overload_operator_from_tuple_ {using type = nt_overload_operator_<OutArg>;};

template<typename OutArg, typename... Args>
struct nt_overload_operator_from_tuple_<OutArg, std::tuple<Args...>> {using type = nt_overload_operator_<OutArg, Args...>;};
...
struct nt_overload_operator_{
    ...
    //but for example;
    //(int, nt_convertible_<nt::Scalar>, nt_auto_)
    //can take:
    ...
    inline static constexpr bool is_callable(){
        ...
        return nt_is_callable_<ParamArgs...>::template value<Type>();
    }
    ...
    inline static constexpr bool is_convertible_callable(){
        ...
        static_assert(sizeof...(ParamArgs) <= std::tuple_size_v<Type>,
                      "[POTENTIAL INTERNAL ERROR] input types cannot have less than there are parameters for overloading a function");
        return nt_is_callable_<ParamArgs...>::template convertible_value<Type>();
    }
    
    template<class Tuple>
    using fix_overload_type = typename nt_overload_operator_from_tuple_<typename nt_extract_auto_return_type_<OutArg, Tuple>::type, typename nt_fix_arg_holder_<Tuple, std::tuple<ParamArgs...>>::type>::type;


    static constexpr bool has_auto = _hasnt_auto_<ParamArgs...>::value;
    static constexpr bool has_convertible = _hasnt_convertible_<ParamArgs...>::value;
    
    
};

template<typename T>
struct _isnt_overload_operator_ : std::false_type {};

template<typename... Args>
struct _isnt_overload_operator_<nt_overload_operator_<Args...>> : std::true_type {};

template<typename... Args>
struct _all_isnt_overload_operator_ : std::true_type {};

template<typename Arg1, typename... Args>
struct _all_isnt_overload_operator_<Arg1, Args...>{
    static constexpr bool value = _isnt_overload_operator_<Arg1>::value && _all_isnt_overload_operator_<Args...>::value; 
};


template<std::size_t i, std::size_t out, bool found, bool convertible, typename Tup, typename... Args>
inline constexpr std::size_t nt_find_valid_overload_operator_(){
    if constexpr(i == sizeof...(Args)){return out;}
        ...
        if constexpr (convertible){
            ...
            return nt_find_valid_overload_operator_<i+1, out, found, convertible, Tup, Args...>();
        }else{
            ...
            if constexpr (std::tuple_element_t<i, std::tuple<Args...>>::template is_callable<Tup>()){
                if constexpr (std::tuple_element_t<i, std::tuple<Args...>>::has_auto){
                    return nt_find_valid_overload_operator_<i+1, out, found, convertible, Tup, Args...>(); //pass
                }
                else if constexpr (std::tuple_element_t<out, std::tuple<Args...>>::has_auto){
                    return nt_find_valid_overload_operator_<i+1, i, found, convertible, Tup, Args...>(); //pass
                }else{
        ...
        if constexpr (std::tuple_element_t<i, std::tuple<Args...>>::template is_convertible_callable<Tup>()){
            return nt_find_valid_overload_operator_<i+1, i, true, true, Tup, Args...>(); 
        }else{
            return nt_find_valid_overload_operator_<i+1, out, found, convertible, Tup, Args...>();
        }
    }else{
        if constexpr (std::tuple_element_t<i, std::tuple<Args...>>::template is_callable<Tup>()){
            return nt_find_valid_overload_operator_<i+1, i, true, false, Tup, Args...>(); 
        }else{
            return nt_find_valid_overload_operator_<i+1, out, found, convertible, Tup, Args...>();
        }
    }
...
template<typename... Args>
struct nt_multi_overload_operator_{
    static_assert(_all_isnt_overload_operator_<Args...>::value,
                  "[INTERNAL LOGIC ERROR] Expected to parse multiple overload operators");

    static constexpr std::size_t Size = sizeof...(Args);
    
    template<typename Tup>
    inline static constexpr std::size_t callable_convertible_index(){return nt_find_valid_overload_operator_<0, Size, false, true, Tup, Args...>();}
    template<typename Tup>
    inline static constexpr std::size_t callable_index(){return nt_find_valid_overload_operator_<0, Size, false, false, Tup, Args...>();}
...
//     using overload_type =
//         nt_multi_overload_operator_<
//             nt_overload_operator_<void, int, int, int, double>,
//             nt_overload_operator_<void, float, int, int, double>,
//             nt_overload_operator_<nt_auto_return_<0>&, nt_auto_&, nt_convertible_<float>, int, double>
//         >;
